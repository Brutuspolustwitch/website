...
-- External Offer Sites (admin-controlled feeds for other websites)
...
create table if not exists page_settings (
  ...
  hero_text_align text not null default 'left' check (hero_text_align in ('left', 'center', 'right')),
  hero_position_x integer not null default 6 check (hero_position_x >= 0 and hero_position_x <= 100),
  hero_position_y integer not null default 32 check (hero_position_y >= 0 and hero_position_y <= 100),
  hero_max_width integer not null default 768 check (hero_max_width >= 300 and hero_max_width <= 1920),
